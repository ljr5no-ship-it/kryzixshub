...
YT_BRIDGE_HTML = r"""<!DOCTYPE html><html><head><meta charset="utf-8">
<meta name="referrer" content="strict-origin-when-cross-origin"><title>Player</title>
...
<iframe id="p" referrerpolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen
 src="https://www.youtube-nocookie.com/embed/{{ID}}?autoplay=1&rel=0&playsinline=1&enablejsapi=1&origin={{ORIGIN}}"></iframe>
<script>
// Tell the hub if YouTube reports an embed error so it can fall back to Invidious/Piped.
window.addEventListener('message',function(e){
  ...
  if(d&&d.event==='onError'&&window.parent!==window){window.parent.postMessage({kxYtError:d.info,id:'{{ID}}'},'*');}
});
document.getElementById('p').addEventListener('load',function(){
  try{this.contentWindow.postMessage(JSON.stringify({event:'listening',id:1}),'*');}catch(x){}
});
</script></body></html>"""
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif url.path.startswith("/yt/"):
            # Error 153 bridge: serve the embed from a real http:// origin so YouTube
            # receives a valid Referer/origin (file:// pages send none).
            vid = url.path[4:].split("/")[0]
            if not re.fullmatch(r"[A-Za-z0-9_-]{11}", vid or ""):
                self.send_response(400)
                self._send_cors()
                self.end_headers()
                self.wfile.write(b'{"error": "bad video id"}')
                return
            origin = f"http://{self.headers.get('Host') or '127.0.0.1:4545'}"
            page = YT_BRIDGE_HTML.replace("{{ID}}", vid).replace("{{ORIGIN}}", origin)
            self.send_response(200)
...
def _open_hub_when_ready(port):
    """Wait until the hub answers on /health, then open it in the default browser."""
    ...
    for _ in range(40):
        try:
            ...
            time.sleep(0.25)
    webbrowser.open(url)
    print(f"[API] Opened hub: {url}")


def start_api_server(port=4545, open_browser=False):
    class ReusableServer(socketserver.ThreadingMixIn, HTTPServer):
        allow_reuse_address = True
        daemon_threads = True

    if open_browser:
        threading.Thread(target=_open_hub_when_ready, args=(port,), daemon=True).start()
    try:
        ...
        # Usually means another copy is already running; the browser opener above
        # will still find that instance and open the hub.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--open", action="store_true", help="Open the local hub in the browser once the server is ready")
    args, _ = parser.parse_known_args()

    if args.server:
        print("Starting in headless server mode...")
        start_api_server(args.port, args.open)
    else:
        # Start server bridge in daemon thread alongside GUI
        srv_thread = threading.Thread(target=start_api_server, args=(args.port, args.open), daemon=True)
        srv_thread.start()
